...
// considers only a submatrix with n_col
							...
							int n_col)
{
	...
	copy = isl_mat_sub_alloc(mat, 0, mat->n_row, 0, n_col);
	copy = isl_mat_cow(copy);

	if (isl_mat_rank(copy) == n_col) {
		isl_mat_free(copy);
		return mat;
	}
	...
	H = isl_mat_left_hermite(isl_mat_dup(copy), 0, NULL, NULL);
