...
__isl_give isl_mat *orthogonal_complete(__isl_take isl_mat *mat, int row)
{
	...
	for ( ; row < n_col; ++row) {
		isl_vec *new_row;
		struct isl_region full_region = { 0, 2 * n_col };
...
void isl_schedule_constraints_dump(__isl_keep isl_schedule_constraints *sc)
{
	...
	fprintf(stderr, "counted accesses: ");
	isl_union_map_dump(sc->counted_accesses);
	...
	__isl_keep isl_schedule_constraints *sc)
{
	...
	if (init_id_list(graph, sc->constraint[isl_edge_spatial_proximity]) < 0)
		return isl_stat_error;
	if (copy_filter_counted_accesses(graph, sc->counted_accesses) < 0)
		return isl_stat_error;
...
static isl_stat add_intra_proximity_constraints(struct isl_sched_graph *graph,
	struct isl_sched_edge *edge, int s, int local, int pos/*1*/, int start/*5*/)
{
	int offset;
	unsigned nparam;
	isl_map *map = isl_map_copy(edge->map);
	...
	if (!local) {
		isl_dim_map_range(dim_map, pos, 0, 0, 0, 1, 1);
	...
	int s, int local)
{
	...
	coef = intra_coefficients(graph, node, isl_map_copy(map));
	offset = coef_var_offset(coef);
	coef = isl_basic_set_transform_dims(coef, isl_dim_set,
			offset, isl_mat_copy(node->cmap));
	if (!coef)
...
static int add_all_proximity_constraints(struct isl_sched_graph *graph,
	int use_coincidence, int pos/*1*/, int start/*5*/)
{
	int i;
	...
	for (i = 0; i < graph->n_edge; ++i) {
		...
		if (edge->src == edge->dst &&
		    add_intra_proximity_constraints(graph, edge, 1, local, pos, start) < 0)
			return -1;
		if (edge->src != edge->dst &&
		    add_inter_proximity_constraints(graph, edge, 1, local, pos, start) < 0)
			return -1;
		if (is_validity(edge) || local)
			continue;
		if (edge->src == edge->dst &&
...
static isl_stat add_id_to_list(void **element, void *user)
{
	isl_id_list *list = *(isl_id_list **) user;
	isl_id *id = *element;

	list = isl_id_list_add(list, id);
	*(isl_id_list **) user = list;
	return isl_stat_ok;
}
	...
	int use_coincidence)
{
	...
	if (!spatial_locality) {
		if (add_all_proximity_constraints(graph, use_coincidence, 1, 5) < 0)
			return isl_stat_error;
	} else {
